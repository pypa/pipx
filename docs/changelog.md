...
- Changed default nox and Github Workflow tests to use local pypi server with fixed lists of available packages.  This allows greater test isolation (no network pypi access needed) and determinism (fixed available dependencies.)  The old style tests that use the internet to access pypi are still available using `nox -s tests_internet`.  To use the local pypi server with a plain pytest invocation use `pytest --pypiserver tests`.
