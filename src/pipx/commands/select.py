...
def select(
    ...
    if package_name_with_suffix == package_metadata.package:
        raise PipxError(f"Cannot select '{package_name_with_suffix}' as it already has no suffix.")
    ...
    if venv_non_suffix.is_dir():
        raise PipxError(f"Package '{package_metadata.package}' is already installed without suffix.")
